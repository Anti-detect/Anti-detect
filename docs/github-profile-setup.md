...
Steps to maximize visibility for **Anti-detect** user profile and this special repository.

## 1. Repository About (required)
...
|---------|----------------|
| Issues | On (documentation feedback) |
| Discussions | Optional — community Q&A |
| Wiki | Off (keep docs in `/docs`) |
| Sponsorship | Configure via `.github/FUNDING.yml` |
...
Run [scripts/set-github-about.ps1](../scripts/set-github-about.ps1) with a `GITHUB_TOKEN` that has `repo` scope.
